...
class MovieLens1MDatasetWithMetadata(Dataset):
    def __init__(self, ratings_path, users_path='users.dat', movies_path='movies.dat'):
        ratings_df = pd.read_csv(ratings_path, sep='::', engine='python', header=None, names=['user_id', 'movie_id', 'rating', 'timestamp'], encoding='latin-1')
        ...
        # Users metadata
        users_df = pd.read_csv(users_path, sep='::', engine='python', header=None, names=['user_id', 'gender', 'age', 'occupation', 'zip'], encoding='latin-1')
        users_df['age_bin'] = pd.cut(users_df['age'], bins=[1,18,25,35,45,50,56,100], labels=range(7), include_lowest=True)
        ...
        movies_df = pd.read_csv(movies_path, sep='::', engine='python', header=None, names=['movie_id', 'title', 'genres'], encoding='latin-1')
        
        self.movie_titles = {
            self.movie_map[row['movie_id']]: row['title']
